...
songplay_table_create = ("""CREATE TABLE IF NOT EXISTS songplays(songplay_id SERIAL PRIMARY KEY, start_time timestamp not null, user_id int not null, song_id varchar, artist_id varchar, session_id int not null, location varchar, user_agent varchar, level varchar);
""")
...
artist_table_create = ("""CREATE TABLE IF NOT EXISTS artists(artist_id varchar PRIMARY KEY, name varchar not null, location varchar not null, latitude numeric, longitude numeric);
""")
...
songplay_table_insert = ("""INSERT INTO songplays( start_time, user_id, song_id, artist_id, session_id, location, user_agent, level) VALUES(%s, %s, %s, %s, %s, %s, %s, %s) ON CONFLICT (songplay_id) DO NOTHING
""")
...
song_select = ("""select s.song_id, s.artist_id from songs s left join artists a on a.artist_id = s.artist_id where s.title =%s or a.name = %s or duration =%s
""")
